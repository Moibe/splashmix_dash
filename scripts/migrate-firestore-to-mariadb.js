/**
 * Script para migrar registros de Firestore (log_ig) a MariaDB mediante API
 * Uso: node scripts/migrate-firestore-to-mariadb.js
 * 
 * Reutiliza:
 * - Credenciales Firebase del .env
 * - Endpoint /registrar de la API de FastAPI
 */
...
async function main() {
  console.log('🚀 Iniciando migración de Firestore a MariaDB...\n');

  try {
    // 1. Leer todos los documentos de log_ig
    console.log('📖 Leyendo documentos de log_ig...');
    const logSnapshot = await db.collection('log_ig').get();
    console.log(`✅ Se encontraron ${logSnapshot.size} registros en log_ig\n`);
    ...
    // 3. Procesar cada registro de log_ig y enviarlo a la API
    ...
    for (let i = 0; i < registrosArray.length; i++) {
      try {
        ...
        // Preparar payload para la API (igual a lo que hace la app)
        const payload = {
          uid: userData.uid,
          ...
          proveedor: logData.proveedor || 'FLUX',
          prompt_type: logData.prompt_type || '',
          prompt_eval: logData.prompt_eval || '',
        ...
        if (response.ok) {
          ...
          console.warn(`⚠️ Error al insertar registro: ${response.status} ${response.statusText}`);
